...
\ea\label{ex:zimmermann:2}
    \ea\label{ex:zimmermann:2a}\gll Man muss dem (, dass Peter faul ist)\textsubscript{\textalpha} zustimmen (, dass Peter faul ist)\textsubscript{$-$\textalpha}.\\
...
The respective forms of the correlate as well as the syntactic and semantic types of the embedded clause are determined by the embedding lexical head -- verbs as in \REF{ex:zimmermann:3.1}/\REF{ex:zimmermann:3.2}, adjectives as in \REF{ex:zimmermann:4.1}/\REF{ex:zimmermann:4.2}, and prepositions as in \REF{ex:zimmermann:5.1}/\REF{ex:zimmermann:5.2}.\footnote{In \ili{German}, the lexical heads V and A are XP-final, in \ili{Russian} they are XP-initial. Nouns deserve a special treatment (see below). See \citet{Knjazev2016} who raises fundamental questions with respect to nominalizations.}\textsuperscript{,}\footnote{In \REF{ex:zimmermann:3.1}--\REF{ex:zimmermann:5.2}, the pronoun \textit{wer/kto} `who' represents clauses with initial w/k-phrases. Note that \textit{wie/kak}(`how')-clauses can be embedded by predicates of perception \citep[see][]{Zimmermann1991}, and that all subjunctions in \REF{ex:zimmermann:5.1}/\REF{ex:zimmermann:5.2} introduce adverbial clauses.}
...
\ea\label{ex:zimmermann:7}
    ...
    \ex\label{ex:zimmermann:7b} \Big[\textsubscript{XP} X\textsubscript{\textalpha} (\big[\textsubscript{PP} P)\textsubscript{\textbeta} [\textsubscript{DP} [\textsubscript{DP} [\textsubscript{D$'$} [\textsubscript{D} \big\{\{\textit{es}/\textit{das}\}, \textit{ėto}\big\}]]](])\textsubscript{\textbeta} (CP)\big] X\textsubscript{$-$\textalpha}\Big]
\z\z
...
\ea\label{ex:zimmermann:16}
    \ea\label{ex:zimmermann:16a} /\{zufrieden/dovolen\textsubscript{\textalpha}\}/

    \ex\label{ex:zimmermann:16b} $\lbrack+$V,$+$N,($-$fem,$-$neuter,$-$pl)\textsubscript{\textalpha}$\rbrack$

    \ex\label{ex:zimmermann:16c} $(\lambda d) . (\lambda x$\textsubscript{[\{mit/$+$R$+$P$-$U\};($-$interr$-$dir/$+$def$+$interr$+$wh)]}$) . \lambda z \big[[(d) = (N)] \, \wedge$ $[\textsc{content-with} (d) (x) (z)]\big]$ $\newline {\small \textsc{content-with} \in \stb{d\stb{\beta\stb{{et}}}}, {\beta} \in \{e,st\}}$
\z\z
...
\ea\label{ex:zimmermann:17}
    \ea /\{Frage\textsubscript{\textalpha}/vopros\textsubscript{\textbeta}\}/ \label{ex:zimmermann:17a}

    \ex $\lbrack +$N,$-$V,\textalpha fem,\textbeta masc,$-$pl,$\lbrace$\textgamma governed,$-$oblique/\textgamma R,$-$P,$-$U$\rbrace\rbrack$ \\
    {\small \textalpha{} $= + \rightarrow$ \textbeta{} $= -$, \textbeta{} $= + \rightarrow$ \textalpha{} $= -$} \label{ex:zimmermann:17b}

    \ex $\lambda x_{[-\textrm{def}+\textrm{interr}]} \, [\textsc{question} (x)] \, {\small \in \stb{et}}$\label{ex:zimmermann:17c}
\z\z
...
\ea\label{ex:zimmermann:18}
    ...
    \ex\label{ex:zimmermann:18c} $\lambda P$\textsubscript{[\textbeta V\textgamma N]}$. \lambda x . \lambda e \big[(e) \, \cnst{inst} \, [P (x)]\big] \, {\small \in \stb{\alpha t\stb{\alpha\stb{{et}}}}}$ \\
...
\ea\label{ex:zimmermann:20} \sib{\{\textit{damit zufrieden, dass/wer {\dots}} / \textit{dovolen tem, čto/kto {\dots}}\} `content with that/who {\dots}' } $=$
\ref{ex:zimmermann:16c}\Big(\ref{ex:zimmermann:15c}\big(\ref{ex:zimmermann:19}(\sib{CP})\big)\Big) $=$\\
...
\ea\label{ex:zimmermann:21} $\lambda z . \lambda y \, [\textsc{consist-in} (z) (y)]$ {\small $\in \stb{st\stb{et}}$} \hfill (TS\textsubscript{PM2}) \z
...
\ea\label{ex:zimmermann:22} \sib{\{\textit{die Frage,} \{\textit{ob Peter}/\textit{wer}\} \textit{gewonnen hat} / \textit{(tot) vopros,} $\{$\textit{pobedil li Pëtr}/\textit{kto pobedil}$\}\}$ `the question \{whether Peter/who won\}'} $=$ \\
\ref{ex:zimmermann:15c}\Big(\sib{\{\textit{Frage}/\textit{vopros}\}}\big(\ref{ex:zimmermann:21}(\sib{CP})\big)\Big) $=$ \\
...
\sib{\{\textit{mit}/\textit{s}\}} \Big(\ref{ex:zimmermann:15c} (\sib{\{\textit{Ziel}/\textit{cel'}\textsubscript{\textalpha}\}}) \big(\ref{ex:zimmermann:21} (\sib{CP})\big)\Big) $=$ \newline
...
\sib{\{\textit{damit}/\textit{s tem}\textsubscript{\textalpha}\}} \big(\ref{ex:zimmermann:21} (\sib{CP})\big) $=$\\
$\lambda e. \Big[(e) \, R \, \Big(\iota x \big[[\textsc{aim} (x)] \wedge [\textsc{consist-in} ($\sib{CP}$) (x)]\big]\Big)\Big] \in \stb{et}$
\z
...
\ea\label{ex:zimmermann:28.2}\label{ex:zimmermann:28.b}
...
\noindent With the semantics of the copula and the functional categories of the matrix-clause we get \REF{ex:zimmermann:31} as the SF of the examples in \REF{ex:zimmermann:28b}. The peculiarities of the syntax and semantics of the functional CP-domains need not interest us here (on the syntax see \REF{ex:zimmermann:6}). Attention should be paid to the semantic amalgamation of the copula with the predicative and the nominalized propositional subject.

    \begin{doublespace} % looks better (see above already)

\ea\label{ex:zimmermann:31}
