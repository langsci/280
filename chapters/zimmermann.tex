...
\ea\label{ex:zimmermann:15}
    ...
    $/\big\{\{es_{\alpha}/das\}/to/\varnothing\big\}/, ([_\textrm{DP} \, \_\_ \, ])$\textsubscript{α}

    \ex\label{ex:zimmermann:15b}
    $\lbrack+$D,$+$def,$+$dem,$-$anaph,βgiven,$-$I,$-$II,$-$pl,$-$fem,$-$masc, \newline $\lbrace$ɣgoverned,$-$oblique/ɣR,$-$P,$-$U$\rbrace\rbrack$
...
\ea\label{ex:zimmermann:17}
    ...
    \ex $\lbrack +$N,$-$V,αfem,βmasc,$-$pl,$\lbrace$ɣgoverned,$-$oblique/ɣR,$-$P,$-$U$\rbrace\rbrack$ \\
    {\small $\alpha= + \rightarrow \beta = -, \beta = + \rightarrow \alpha = -$} \label{ex:zimmermann:17b}
...
\ea\label{ex:zimmermann:18}
    ...
    \ex\label{ex:zimmermann:18c} $\lambda P$\textsubscript{[βVɣN]}$. \lambda x . \lambda e \big[(e) \, \cnst{inst} \, [P (x)]\big] \, {\small \in \stb{\alpha t\stb{\alpha\stb{{et}}}}}$ \\
    {\small $\alpha{} \in \{st, t, e, i, {\ldots} \}, \beta = + \rightarrow \gamma = +$}
\z\z
...
\sib{\{\textit{mit}/\textit{s}\}} \Big(\ref{ex:zimmermann:15c} (\sib{\{\textit{Ziel}/\textit{cel'}\textsubscript{α}\}}) \big(\ref{ex:zimmermann:21} (\sib{CP})\big)\Big) $=$ \newline
...
\sib{\{\textit{damit}/\textit{s tem}\textsubscript{α}\}} \big(\ref{ex:zimmermann:21} (\sib{CP})\big) $=$\\
$\lambda e. \Big[(e) \, R \, \Big(\iota x \big[[\textsc{aim} (x)] \wedge [\textsc{consist-in} ($\sib{CP}$) (x)]\big]\Big)\Big] \in \stb{et}$
\z
