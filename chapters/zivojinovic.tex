...
\begin{document}
\shorttitlerunninghead{Torlak clitic doubling: A cross-linguistic comparison}
\maketitle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%       Chapter 1
...
CLRD occurs in Torlak as well, being the complementary structure to CD. Indeed, example \REF{ex:7} in Bulgarian has its equivalent formation:\footnote{Torlak does not make a distinction between proximal = V, neutral = T and distal = N articles, as Macedonian does.}
...
\noindent Other options which are present in Bulgarian, namely HTLD, CLLC, are lacking in Torlak. In fact, the equivalent Torlak examples of \REF{ex:8} and \REF{ex:9}, illustrated in \citet{Cinque.Krapova2008}, are ungrammatical.
...
\noindent In fact, we find the following examples of a maximal projection as in \REF{ex:19} or a prosodic word as in \REF{ex:20}.
...
\noindent Second position clitics are to be found in different types of configurations: in verb-initial clauses as in \REF{ex:21} and with a clitic in first position as in \REF{ex:22}.
...
\noindent The clitic-first configuration in \REF{ex:21} illustrates one of the two possible exceptions to the second-position placement. Namely, clitics as unstressed particles cannot occur in the first position. However, the clitic \textit{je} has a stressed counterpart, making it a non-clitic, according to \citet[226]{Franks.King2000}. It is followed by the question clitic \textit{li}, which occurs in the typical second position.
...
In Bulgarian, clitics precede finite verbs (except when the finite verb is in the first position). This means that clitics can be placed in any position in the sentence, except for the first one; see \REF{ex:24}.
...
When it comes to the variation in clitic placement, Torlak surely stands somewhere in between the above-mentioned possible scenarios. Because Torlak allows clitic doubling, as exemplified in \REF{ex:27}, one might be tempted to assume that word order in clitic constructions might resemble either Bulgarian or Macedonian. But let us check some examples and counter-examples:
...
\noindent Unlike examples \REF{ex:29} and \REF{ex:30}, example \REF{ex:31} displays a configuration involving a verb-initial construction. Just as in previous cases, the clitic appears in the second position (as in example \REF{ex:28}).
...
{\gll Ga         poznavam Milovana.\\
     him.\textsc{cl.acc}  know.\textsc{1sg}     Milovan\\\label{ex:33}
...
{\gll Odamna            Milovana   ga         upozna.\\
    long.time.ago  Milovan    him.\textsc{cl.acc} met.\textsc{1sg} \\\label{ex:38}
...
\noindent It emerges from the above-listed examples that configurations which are allowed in both Bulgarian (see \REF{ex:38} where the clitic is in the third position and precedes the main verbs) and Macedonian (see \REF{ex:33}, clitic-first construction) are not acceptable in Torlak. On the other hand, as examples \REF{ex:35} and \REF{ex:37} show, Torlak allows non-verb-adjacent clitics, unlike Bulgarian and Macedonian. Just as Serbo-Croatian, it supports the use of clitics after the first prosodic word (example \REF{ex:39}), following \citet{Boskovic2001} and \citet{Radanovic-Kocic1988}.
