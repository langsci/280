...
\begin{document}
\shorttitlerunninghead{From measure predicates to count nouns}
\maketitle
...
Colloquial Russian uses productively morphologically complex measure nouns. These are nouns constructed out of a numeral, a measure word, and a nominal suffix -\textit{ka} \REF{ex:1}.
...
\noindent Such nouns apparently look like measure expressions such as \textit{tri litra/trex litrov} `three liters' used in pseudo-partitives such \textit{tri litra/trex litrov vody} illustrated in \REF{ex:2}.
...
\noindent However, the two constructions are very different. While \textit{three liters} in \REF{ex:2} expresses measure properties of entities, the \textsc{measure nouns} in \REF{ex:1} denote actual objects (glasses, jars etc..) that have these properties. As further shown in \REF{ex:3}, these nouns have sortal uses and can be modified by adjectives. They cannot be used as adjectival modifiers of other nouns \REF{ex:4}.
...
\ea\label{ex:6}
    ...
    \ex\label{ex:6b} \gll Segodnja odela sto-grammov-ku poverx svitšota.\\
    ...
    \ex\label{ex:6c} \gll 56 grammovye šokoladnye plitk-i po forme i ob''emu napominajuščie starye
    sto-grammov-ki\\
...
[tri \hspace{33pt}litr-a]                                                            & $\rightarrow$ & [trex \hspace{24pt}litr-ov]  & $\rightarrow$                                       & [trex-litr-ov-ka]\\
\hspace{4pt}three.\textsc{\small nom} liter-\textsc{\small gen.sg}      &               & \hspace{4pt}three.\textsc{\small gen} liter-\textsc{\small gen.pl}    && \hspace{4pt}three.\textsc{\small gen}-liter-\textsc{\small gen.pl}-\textsc{\small ka}\\
`three liters (NP)'                                                     &               & `(of) three liters'                                                   && `a three-liter jar'\\
\end{tabularx}}
\caption{Numeral NP-to-measure noun-pattern}\label{ex:7}
...
%\resizebox{0.75\textwidth}{!}{
%\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
%\textsc{nominative np}                                              &                       & \textsc{adjective}         &                                                          & \textsc{complex noun}\\

%[tri litr-a]                                                        & $\rightarrow$         & [trex-litr-ov-yj] & $\rightarrow$                                                     & [trex-litr-ov-ka]\\
%\hspace{4pt}three.\textsc{\small nom} liter-\textsc{\small gen.sg}  &                       & \hspace{4pt}three.\textsc{\small gen}-liter-\textsc{\small adj}-\textsc{\small m.sg} && \hspace{4pt}three.\textsc{\small gen}-liter-\textsc{\small gen.pl}-\textsc{\small ka}\\
%`three liters' (NP)                                                 &                       & `three-liter'     &                                                                   & `a three liter jar'
%\end{tabularx}}
%\caption{Numeral adjective-to-measure noun-pattern}\label{ex:8}
%\end{figure}


%%%% NEW FIGURE 2: ALIGNED
\begin{figure}
\resizebox{0.83\textwidth}{!}{
\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
\textsc{nominative np}                                              &                       & \textsc{adjective}         &                                                          & \textsc{complex noun}\\

[tri \hspace{33pt}litr-a]                                                        & $\rightarrow$         & [trex-litr-ov-yj] & $\rightarrow$                                                     & [trex-litr-ov-ka]\\
\hspace{4pt}three.\textsc{\small nom} liter-\textsc{\small gen.sg}  &                       & \hspace{4pt}three.\textsc{\small gen}-liter-\textsc{\small adj}-\textsc{\small m.sg} && \hspace{4pt}three.\textsc{\small gen}-liter-\textsc{\small gen.pl}-\textsc{\small ka}\\
`three liters' (NP)                                                 &                       & `three-liter'     &                                                                   & `a three liter jar'
\end{tabularx}}
\caption{Numeral adjective-to-measure noun-pattern}\label{ex:8}
...
I shall now bring evidence that both patterns occur. In particular, we shall see that there are cases which can be analyzed only as being derived from numeral NPs as in \figref{ex:7} and, conversely, there are measure nouns for which only the `adjective-to-noun' pattern in \figref{ex:8} is possible.

We start with the pattern in \figref{ex:8}. This pattern is very clearly exemplified by \textit{(odno)litrovka} `a one-liter jar/bottle'. The complement of the genitive NP \textit{odnogo litra} `of one liter' is singular and does not have the suffix \textit{-ov}. Therefore deriving `one-liter jar' in the pattern in \figref{ex:7}, i.e. from a measure phrase, would produce \textit{(odno)litrka}, which does not exist (\figref{ex:9}). The \textit{-ov-} suffix in \textit{(odno)litrovka} must come from the adjective \textit{litrovyj} `one-liter'. Thus the most plausible derivation for \textit{litrovka} is from the adjectival base, i.e. through the pattern in \figref{ex:8}, as shown in \figref{ex:10}.\footnote{Examples such as \textit{pjatiletka} `a five-year old/a five-year program' have also been treated as derived from adjectives. Such an analysis assumes that deletion of the adjectival suffix \textit{-n-} takes place, \textit{pjatiletnij} (\textsc{adj}) -- \textit{pjatiletka} (\textsc{noun}) (\citealt{Townsend1975} as opposed to \citealt{Vinogradov1960}).}

%\begin{figure}
%\resizebox{0.75\textwidth}{!}{
%\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
%[odin litr]                                                             & $\rightarrow$ & [(odn-ogo) litr-a]                    & $\rightarrow$             & \minsp{*} [(odno)-litr-ka]\\
%\hspace{4pt}one.\textsc{\small nom} liter.\textsc{\small nom.sg}        &               & \hspace{8pt}one-\textsc{\small gen} liter-\textsc{\small gen.sg}  && \hspace{12pt}one.\textsc{\small gen}-liter-\textsc{\small ka}\\
...
%%%% NEW FIGURE 3: ALIGNED
\begin{figure}
\resizebox{0.85\textwidth}{!}{
\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
[odin \hspace{17pt}litr]                                                             & $\rightarrow$ & [(odn-ogo) litr-a]                    & $\rightarrow$             & \minsp{*} [(odno)-litr-ka]\\
\hspace{4pt}one.\textsc{\small nom} liter.\textsc{\small nom.sg}        &               & \hspace{8pt}one-\textsc{\small gen} \hspace{4pt}liter-\textsc{\small gen.sg}  && \hspace{12pt}one.\textsc{\small gen}-liter-\textsc{\small ka}\\
`one liter'                                                             &               & `of one liter'                         &                           & `a one-liter jar'\\
\end{tabularx}}
\caption{Numeral NP-to-measure noun-pattern (ungrammatical)}\label{ex:9}
\end{figure}
...
%\resizebox{0.75\textwidth}{!}{
%\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
%[odin litr]                                                             & $\rightarrow$ & [(odn-o)-litr-ov-yj]                 & $\rightarrow$                                 & [(odn-o)-litr-ov-ka]\\
%\hspace{4pt}one.\textsc{\small nom} liter-\textsc{\small nom.sg}        &               & \hspace{8pt}one-liter-\textsc{\small adj}-\textsc{\small m.sg}  && \hspace{8pt}one-liter-\textsc{\small adj}-\textsc{\small ka}\\
%`one liter'                                                             &               & `of one liter'                         &                                               & `a one-liter jar'\\
%\end{tabularx}}
%\caption{}\label{ex:10}
%\end{figure}

%%%% NEW FIGURE 4: ALIGNED
\begin{figure}
\resizebox{0.85\textwidth}{!}{
\begin{tabularx}{\textwidth}{@{}lXlXl@{}}
[odin \hspace{17pt}litr]                                                             & $\rightarrow$ & [(odn-o)-litr-ov-yj]                 & $\rightarrow$                                 & [(odno)-litr-ov-ka]\\
\hspace{4pt}one.\textsc{\small nom} liter-\textsc{\small nom.sg}        &               & \hspace{8pt}one-liter-\textsc{\small adj}-\textsc{\small m.sg}  && \hspace{8pt}one.\textsc{\small gen}-liter-\textsc{\small adj}-\textsc{\small ka}\\
`one liter'                                                             &               & `of one liter'                         &                                               & `a one-liter jar'\\
\end{tabularx}}
\caption{Numeral adjective-to-measure noun-pattern (ungrammatical)}\label{ex:10}
\end{figure}

Evidence for the pattern in \figref{ex:7} comes from the contrast between \textit{sto\-gramm\-ka} in \REF{ex:11a}, \REF{ex:11c}, and \textit{stogrammovka} in \REF{ex:11b}, earlier illustrated in \REF{ex:1b} and \REF{ex:6}. While the noun phrase `hundred grams' has two productive variants, one with the \textit{-ov-} suffix in \REF{ex:12a} and one without it \REF{ex:12b}, the adjective `hundred-gram' has only one productive form which is derived using the adjectival suffix \textit{-ov-} \REF{ex:13}. Therefore, \textit{stogrammovka} could be derived either from the adjectival form \textit{stogrammovyj} in \REF{ex:13a} or from the measure phrase \textit{sto grammov} \REF{ex:12a}. \textit{Stogrammka}, which lacks \textit{-ov-}, however, is most plausibly derived from the measure phrase \textit{sto gramm} in \REF{ex:12b}, since the adjectival form \textit{stogrammnyj} in \REF{ex:13b} is not productive. I did find few occurrences of this form on the Internet, but all my informants who are ready to accept \textit{stogrammka} (even though this form is quite rare, too) reject it. At least for those speakers, \textit{stogrammka} must be derived from a nominal phrase \textit{sto gramm} and not from the adjective \textit{stogrammovyj}, i.e. via the pattern in \figref{ex:7}.

\ea\label{ex:11}
    \ea\label{ex:11a}\gll sto-gramm-ka\\
...
%\ea\label{ex:12} Measure phrase
%        \ea\label{ex:12a} \gll sto gramm-ov\\
%        100 gram-\textsc{gen.pl}\\
%        \glt `100 grams'
%        \ex\label{ex:12b} \gll sto gramm\\
%        100 gram.\textsc{gen.pl}\\
%    \glt `100 grams'\\
%\z\z
\begin{exe}
\ex\label{ex:12} Measure phrase\vspace{-10pt}
    \begin{multicols}{2}
        \begin{xlist}
            \ex\label{ex:12a} \gll sto gramm-ov\\
            100 gram-\textsc{gen.pl}\\
            \glt `100 grams'
            \ex\label{ex:12b} \gll sto gramm\\
            100 gram.\textsc{gen.pl}\\
...
%\ea\label{ex:13} Adjective
%    \ea\label{ex:13a} \gll sto-gramm-ov-yj\\
%    100-gram-\textsc{adj}-\textsc{m.sg}\\
%    \glt `100-gram'
%    \ex[?]{\label{ex:13b} \gll sto-gramm-n-yj\\
%    100-gram-\textsc{adj}-\textsc{m.sg}\\
%    \glt `100-gram'}
%\z\z

\begin{exe}
\ex\label{ex:13} Adjective\vspace{-10pt}
    \begin{multicols}{2}
        \begin{xlist}
            \ex\label{ex:13a} \gll sto-gramm-ov-yj\\
            100-gram-\textsc{adj}-\textsc{m.sg}\\
            \glt `100-gram'
            \ex[?]{\label{ex:13b} \gll sto-gramm-n-yj\\
            100-gram-\textsc{adj}-\textsc{m.sg}\\\\
...
With \citet{,Rothstein2011}, \citet{Landman2011,Landman2016}, and \citet{Filip.Sutton2016} I assume that the count/mass contrast in the nominal domain amounts to the distinction between disjoint and overlapping denotations. In particular, singular count nouns denote sets of disjoint entities, and plural count nouns denote sets of these disjoint entities closed under sum, whereas mass denotations can be generated by sets of overlapping entities. Measure nouns such as \textit{trexlitrovka}, which have count denotations, will therefore denote sets of disjoint entities.

As for the measure expression, I will base myself on the framework in \citet{Landman2004,Landman2016}, \citet{Rothstein2009,Rothstein2011,Rothstein2017} (for English), and \citet{Partee.Borschev2012}, \citet{Khrizman2016a,Khrizman2016b} (for Russian). In this framework measure phrases such as \textit{100 grams} are intersective modifiers which express measure properties, i.e. properties of having a value on a dimensional scale calibrated in certain units \REF{ex:14}.

\ea\label{ex:14}
    \ea\label{ex:14a} $P\textsubscript{\textsc{meas}} = \lambda x. \textsc{meas}\,\textsubscript{\textsc{dim unit}}\,(x) = n$
    \ex\label{ex:14b} $P\textsubscript{\textsc{100 grams}} = \lambda x. \textsc{meas}\,\textsubscript{\textsc{weight gram}}\,(x) = 100$\\
    {\small the property of having the value 100 on a weight scale calibrated in gram units}
    \z
\z

\noindent \citet{Rothstein2017} showed that measure properties as defined in \REF{ex:14} are expressed by constructions of two types. One, as already mentioned above, is via nominal measure heads such as \textit{100 grams} used in pseudo-partitives like \textit{100 grams of flour} \REF{ex:15a}. The other is via distributive measure adjectives such as \textit{100-gram} in \textit{a 100-gram apple} \REF{ex:16a}. Both expressions denote the property of weighing 100 grams. In \REF{ex:15} this property is assigned to sums of entities denoted by the mass predicate \textit{flour} \REF{ex:15b} and in \REF{ex:16} the same property is assigned to individual apples in the denotation of the count singular \textit{apple} \REF{ex:16b}.\footnote{It is known that the classifier and the adjectival use of measure expressions illustrated in \REF{ex:15} and \REF{ex:16}, respectively, show differences in distribution and interpretation. Classifier uses like those in \REF{ex:15} induce extensive readings, whereas adjectival forms like those in \REF{ex:16} encode non-extensive measure functions. Further, classifier uses are not distributive, whereas adjectival ones are \citep{Schwarzschild2005}. \citet{Rothstein2017} shows that such differences are not an indication of a different semantics of the two expressions but follow from the differences in their syntactic positions.}

    \largerpage[2] % to have (11) in full on this page

\ea\label{ex:15} 100 grams of flour
    \ea\label{ex:15a} \sib{hundred grams} $= \lambda x. \textsc{meas}\,\textsubscript{\textsc{weight gram}}\,(x)= 100$
    \ex\label{ex:15b} \sib{hundred grams of flour} $= \lambda x. \textsc{flour}(x) \wedge \textsc{meas}\,\textsubscript{\textsc{weight gram}}\,(x) = 100$\\
    {\small the set of sums of flour that weigh 100 grams}
    \z
\z

\ea\label{ex:16} a 100-gram apple
    \ea\label{ex:16a} \sib{hundred-gram} $= \lambda x. \textsc{meas}\, \textsubscript{\textsc{weight gram}}\,(x) = 100$
    \ex\label{ex:16b} \sib{a hundred-gram apple} $= \lambda x. \textsc{apple}(x) \wedge \textsc{meas}\,\textsubscript{\textsc{weight gram}}\,(x) = 100$\\
    {\small the set of apples such that each weighs 100 grams}
    \z
\z

\noindent With this background I now propose a basic semantic derivation of morphologically complex measure nouns as follows in \REF{ex:17}. Measure nouns are derived via a nominalization operation, expressed by the \textit{-ka} suffix, which shifts intersective predicate modifiers expressing measure properties to count nouns denoting sets of contextually determined disjoint elements which have these measure properties. \textit{Stogrammovka}, for example, starts off as a measure predicate denoting the property of weighing 100 grams in \REF{ex:18a}. \textsc{-ka} shifts it into a singular count predicate denoting the set of disjoint entities such that each weighs 100 grams \REF{ex:18b}.

\ea\label{ex:17} The semantics of complex measure nouns
    \ea\label{ex:17a} \sib{-ka} $= \lambda P_{\textsc{meas}} \lambda x.N_{c}(x) \wedge P_{\textsc{meas}}(x),$\\
    {\small $N_C$ is a property whose context is contextually determined, $N_C$ is a disjoint set.}
    \ex\label{ex:17b} \sib{measure noun} $= \lambda x. N_{c}(x) \wedge P_{\textsc{meas}}(x),$\\
    {\small $N_C$ is a property whose context is contextually determined, $N_C$ is a disjoint set.}
\z\z

\ea\label{ex:18}
    \ea\label{ex:18a} $P_{\textsc{100 grams}} = \lambda x. \textsc{meas}\,_{\textsc{weight gram}}\,(x) = 100$
    \ex\label{ex:18b} \sib{stogrammovka} $= \lambda P_{\textsc{meas}} \lambda x.N_{c}(x) \wedge P_{\textsc{meas}}(x) (\lambda x.\textsc{meas}\,_{\textsc{weight gram}}\,(x) = 100)$\\
...
The contrast in \REF{ex:20a} and \REF{ex:20b} shows that container interpretations of complex nouns are different from other readings. In particular in \REF{ex:20a} \textit{stogrammovka} refers to a bar whose weight is 100 grams, whereas in \REF{ex:20b} the same noun refers to a glass which can hold 100 grams but does not weigh 100 grams by itself.

\ea\label{ex:20}
    \ea\label{ex:20a} \gll 56 grammovye šokoladnye plitk-i po forme i ob''emu napominajuščie starye sto-grammov-ki\\
    56 gram chocolate bar-\textsc{pl} by form and volume reminding old 100-gram-\textsc{ka.acc.pl}\\
...
\noindent To capture that contrast I follow \citet{Casati.Varzi1999} and \citet{Rothstein2009,Rothstein2017} and treat containers as complex objects which incorporate holes which are themselves objects to which properties can be assigned \REF{ex:21}.
...
\noindent The basic interpretational schema in \REF{ex:17} is then extended to container complex nouns as follows in \REF{ex:22}. Container measure nouns denote sets of contextually disjoint objects that are containers whose holes have a certain measure property in terms of volume \REF{ex:23}.\footnote{In Russian, grams are sometimes used for volume; e.g., \textit{sto gramm(ov) vodki} `100 grams of vodka'.}
...
\ea\label{ex:23}
    \ea\label{ex:23a} \sib{stogrammovka} $= \lambda x. N_{\textsc{container}_{c}}(x) \wedge \textsc{meas}\,_{\textsc{vol gram}}\,(\textsc{hole}(x)) = 100,$\\
    ...
    \ex\label{ex:23b} \sib{trexlitrovka} $= \lambda x. N_{\textsc{container}_{c}}(x) \wedge \textsc{meas}\,_{\textsc{vol liter}}\,(\textsc{hole}(x)) = 3,$\\
...
\noindent Shifts from a measure interpretation to a container interpretation are not unknown. \citet{Khrizman.etal2015} show that lexical measures like \textit{liter} in certain contexts shift to a container reading \REF{ex:24}.
...
\noindent \citet{Khrizman.etal2015} argued that in such cases \textit{liter} is reinterpreted as a container whose contents measures 1 liter in volume \REF{ex:25}.
...
\noindent I do not adopt this for measure nouns, since unlike \textit{liter} they have non-relational uses at type $\stb{e,t}$ \REF{ex:26}, so the measure properties must apply to containers and not to contents. \textit{Trexlitrovka} in \REF{ex:26a} can easily refer to an empty container, whereas \textit{three liters} cannot \REF{ex:26b}.
...
\ea\label{ex:26}
    ...
    \glt `A three-liter jar rolled down to the floor and smashed to pieces.'\label{ex:26a}}
    \ex[*]{\label{ex:26b}
    Three liters broke.
...
\noindent Count nouns have the same ambiguity in Russian, too \REF{ex:28} (see \citealt{Partee.Borschev2012,Khrizman.Rothstein2015,Khrizman2016a,Khrizman2016b}).
...
\noindent If container complex measure nouns are genuine count nouns then we can ask whether they can be used as in pseudo-partitives, and if it is possible, we would expect them to be ambiguous between a classifier and a measure use, too. And this is the case. The examples in \REF{ex:29} illustrate a container classifier use. The semantic interpretations is then as follows in \REF{ex:30}. \textit{Trexlitrovka} shifts from the $\stb{e,t}$ sortal interpretation in \REF{ex:23b} to a relational interpretation in \REF{ex:30a}. It combines with a complement \textit{honey} and creates a predicate denoting the set of disjoint containers which have 3-liter holes and which are filled with honey \REF{ex:30b}.\footnote{The analysis in \REF{ex:30} is based on the analyses of Russian pseudo-partitives with container nouns such as \textit{stakan} `glass' on the classifier use proposed in \citet{Partee.Borschev2012} and \citet{Khrizman2016a,Khrizman2016b}. For details see the original papers.}
...
\ea\label{ex:30}
    \ea\label{ex:30a} \sib{trexlitrovka} $= \lambda P \lambda x.N_{\textsc{container}(x)} \wedge \textsc{meas}\,_{\textsc{vol liter}}\,(\textsc{hole}(x)) = 3 \wedge \exists y [P(y) \wedge \textsc{filled with}(x,y)],$
    {\small $N_{\textsc{container}\textsubscript{c}}$ is disjoint.}
    \ex\label{ex:30b} \sib{trexlitrovka meda} $= \lambda x.N_{\textsc{container}_{c}}(x) \wedge \textsc{meas}\,_{\textsc{vol liter}}\,(\textsc{hole}(x)) = 3 \wedge \exists y [\textsc{honey}(y) \wedge \textsc{filled with}(x,y)],$
    {\small $N_{\textsc{container}\textsubscript{c}}$ is disjoint.}
    \z
{\small the set of contextually determined three-liter containers filled with honey}
\z

\noindent The measure use is more complex. Measure nouns are not used naturally to express standard units of measure \REF{ex:31}. In particular, \textit{trexlitrovka} `three-liter jar' is not used interchangeably with \textit{tri litra} `three liters' to measure out three-liter quantities of $N$. This is presumably expected, since a standard measure expression is available.

\ea\label{ex:31} \gll Zalejte jagody tre-mja litr-ami / \minsp{?} trex-litrov-koj kipjatk-a.\\
...
\noindent But they \textit{are} used as \textit{ad hoc} measure units in approximative contexts (see \citealt{Partee.Borschev2012, Rothstein2017}); see \REF{ex:32}. In \REF{ex:32}, the precise volume of the jar is not directly relevant. The speaker uses the noun not because he knows that this volume corresponds to a certain amount of berries. Instead, the speaker uses the noun to express that he estimates that the amount of the berries on the bush is the amount which would fill a stereotypical three-liter jar. \REF{ex:33} illustrates a similar point.
...
\noindent I thus adopt \citeauthor{Partee.Borschev2012}'s semantics for containers on the \textit{ad hoc} measure interpretation in which a free variable $y$ is used to refer to a container \REF{ex:34}. As a result, the interpretation makes reference to a three-liter container, but does not entail its existence.
...
If complex measure nouns are count predicates and portions are such, too, we predict that measure nouns can denote sets of disjoint portions with certain measure properties. For example, \textit{stogrammovka} could be interpreted as making reference to individual portions which measure 100 grams \REF{ex:37}.

\ea\label{ex:37} \sib{hundred-gram-ka} $= \lambda x.\textsc{portion}_{c} \wedge \textsc{meas}\,_{\textsc{weight/vol gram}}\,(x) = 100$\\
...
\noindent The prediction is borne out. \textit{Frontovaja stogrammovka} illustrated in \REF{ex:38} is a very good example. It is used to refer to a 100-gram portion of vodka which used to be distributed daily to soldiers in the 1940s.
...
\noindent Crucially, portion uses are productive. A Google search reveals a range of contexts in which \textit{stogrammovka} is used to refer neither to containers nor to concrete objects but to abstract portions \REF{ex:39}, \REF{ex:40}.
...
\noindent The second implication relates to the shifting mechanism in the counting and measuring expressions. It is well known that count nouns can shift to denote units of measure. Such shifts, as already mentioned in \sectref{sec:4}, occur in container nouns (\citealt{Doetjes1997,Landman2004,Rothstein2009} and others) as well as in other sortal nouns \REF{ex:44} \citep{Rothstein2017}:

    \largerpage % To have all of (40) on this page
